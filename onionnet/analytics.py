...
#     show_node_counts: bool = False,
#     node_label_fmt: str = "{layer}\n(n={count})",
#     node_text_position: Union[int, float, str] = -1,
...
#     # color + layout
#     family_colors: Optional[Dict[str, Tuple[float,float,float,float]]] = None,
#     family_extractor: Optional[callable] = None,
#     layout: str = "sfdp",
#     show_labels: bool = True,
#     output_size: Tuple[int, int] = (900, 700),
#     return_graph: bool = False,

#     # use custom positioning (e.g. from a previous layout), takes a VertexPropertyMap
#     pos = None,
# ):
#     """
#     Build & draw a tiny meta-graph of layer↔layer connectivity. 
#     Vertices are **layers** and edges represent the **number of edges between layers** in your OnionNet.

#     What it shows
#     -------------
#     • Vertex size ∝ number of nodes in that layer (optional; uniform if `nodes_by_layer=None`).
#     • Edge width ∝ number of edges between layer pairs.
#     • Optional labels:
#         - `show_node_counts=True`: include per-layer node count in the vertex label.
#         - `show_edge_counts=True`: draw the edge count as an edge label.

#     Scaling & ranges
#     ----------------
#     • Geometry is scaled either logarithmically or linearly (`node_scaler`, `edge_scaler`).
#     • Clamp to ranges you control:
#         - Node size: `node_size_range=(min,max)`
#         - Edge width: `edge_width_range=(min,max)`
#         - NEW text sizes: `node_text_size_range`, `edge_text_size_range`

#     Parameters
#     ----------
#     edges_by_pair : DataFrame
#         MultiIndex (source_layer, target_layer) with a single 'edges' column.
#     nodes_by_layer : DataFrame or None
#         Optional per-layer node counts (column 'count'). If None, node sizes are uniform.
#     node_scaler, edge_scaler : {'log','linear'}
#         Transform counts before normalization to the specified ranges.
#     node_size_range, edge_width_range : (float, float)
#         Min/max for node radii and edge pen widths (in pixels/points depending on backend).
#     node_text_size_range, edge_text_size_range : (float, float)
#         Min/max font sizes for node labels and edge labels.
#     show_edge_counts : bool
#         If True, render counts on edges (uses `edge_text` + `edge_font_size`).
#     show_node_counts : bool
#         If True, format node label with `node_label_fmt`.
#     node_label_fmt : str
#         Format for node label when `show_node_counts=True`. Available keys: {layer}, {count}.
#     pad_label_string : bool
#         If True, pad the first line of each node label to the same length (useful for monospaced fonts).
#         This is most helpful when rendering text inside nodes and you want consistent visual widths.
#     node_text_position : float|int|'centered'
#         Position for vertex text; -1 places it inside the vertex. 
#     family_colors : dict
#         Map broad family → RGBA; defaults group swisslipids/sl_* and rhea_*.
#     family_extractor : callable
#         Function to extract a family name from a layer name; defaults to `_infer_family_basic`.
#         If None, uses the layer name directly, grouped by first split prefix. Prefix separators checked (in order): '_', ':', '/', '-', space.
#     layout : {'sfdp'}
#         Layout algorithm (currently sfdp).
#     show_labels : bool
#         Draw layer names (and optionally counts).
#     output_size : (int, int)
#         Canvas size in pixels.
#     pos : VertexPropertyMap or None
#         If provided, use this for vertex positions instead of computing a new layout.
#         This is useful if you want to reuse a layout from a previous graph.
#     use_monospace_font : bool
#         If True, use a monospace font for node/edge text (e.g. "consolas").
#     vertex_font, edge_font : str or None
#         Explicit font names for node and edge text; if None, defaults to monospace if `use_monospace_font=True`.
#     return_graph : bool
#         If True, returns (meta_graph, pos).

#     Notes
#     -----
#     ¹ `graph_tool.draw.graph_draw()` supports vertex/edge text, font sizes, and positioning
#       via `vertex_text`, `vertex_font_size`, `vertex_text_position`, and `edge_text`,
#       `edge_font_size`, `edge_text_parallel`, etc. See the official docs.
#     """
#     import numpy as np
#     from graph_tool.all import Graph, sfdp_layout, graph_draw
...
#     # pick the family color extractor method
#     fam_fn = family_extractor or _infer_family_basic
...
#     # Colors: either use provided palette, or auto-generate distinct colors
#     if family_colors:
#         palette = dict(family_colors)  # user-specified overrides
#         default_rgba = (0.40, 0.40, 0.40, 0.85)
#         for v in mg.vertices():
#             fam = v_family[v]
#             v_color[v] = palette.get(fam, default_rgba)
#     else:
#         # auto-assign from a qualitative colormap
#         import matplotlib.cm as cm
#         fams = sorted({v_family[v] for v in mg.vertices()})
#         cmap = cm.get_cmap("tab20")
#         fam_to_rgba = {f: tuple(cmap(i / max(1, len(fams)-1))) for i, f in enumerate(fams)}
...
#         for v in mg.vertices():
#             v_color[v] = fam_to_rgba[v_family[v]]
#         palette = {**default_family_colors, **(family_colors or {})}
#         for v in mg.vertices():
#             v_color[v] = palette.get(v_family[v], palette.get("other"))
...
#     # build final node text (optionally with counts)
#     if show_labels:
#         if show_node_counts:
#             for v in mg.vertices():
#                 lbl = str(v_label[v])
#                 cnt = layer_counts.get(lbl, 0)
#                 v_label[v] = node_label_fmt.format(layer=lbl, count=cnt)

#         # --- NEW: pad first line of each label to the same length ---
#         if pad_label_string:
#             # collect current labels and first-line lengths
#             labels = [str(v_label[v]) for v in mg.vertices()]
#             first_lines = [(s.splitlines()[0] if "\n" in s else s) for s in labels]
#             max_len = max((len(fl) for fl in first_lines), default=0)
...
#                 s = str(v_label[v])
#                 parts = s.splitlines()
#                 parts[0] = parts[0].ljust(max_len)  # pad first line only
#                 v_label_padded[v] = "\n".join(parts)
#             v_text = v_label_padded
#         else:
#             v_text = v_label
...
#     pos = sfdp_layout(mg) if layout == "sfdp" else pos #sfdp_layout(mg)

#     # --- font selection ---
#     # If the user passed explicit fonts, use them. Otherwise, if the monospace toggle
#     # is on, default to "Monospace". (This is a Pango font family name; e.g. "Monospace",
#     # "Courier New", or "DejaVu Sans Mono" would also work if installed.)
#     v_font = vertex_font if vertex_font is not None else ("consolas" if use_monospace_font else None)
#     e_font = edge_font   if edge_font   is not None else ("consolas" if use_monospace_font else None)
...
#         edge_text_parallel=True,
#         edge_font_size=(e_fsize if show_edge_counts else None),
#         edge_font_family=(e_font if show_edge_counts else None),   # << new
#         output_size=output_size,
#     )

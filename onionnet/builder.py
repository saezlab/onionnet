...
class OnionNetBuilder:
    ...
    def grow_onion(
        ...
        vertex_property_types: dict | None = None,
        edge_property_types: dict | None = None,
        verbose: bool = True,
    ) -> None:
        """
        ...
        if edge_prop_cols is None:
            edge_prop_cols = []
        missing_nodes = {node_id_col, node_layer_col, *node_prop_cols} - set(df_nodes.columns)
        if missing_nodes:
            raise ValueError(f"Missing node columns: {missing_nodes}")
        missing_edges = {
            edge_source_id_col,
            edge_source_layer_col,
            edge_target_id_col,
            edge_target_layer_col,
            *edge_prop_cols,
        } - set(df_edges.columns)
        if missing_edges:
        ...
        self._stats["edges_dropped_invalid"] = self._stats["edges_dropped_na"] + invalid_endpoints

        # optionally display
        if use_display and IPYTHON_AVAILABLE:
            display(df_n_clean.head())
            print("Nodes →", df_n_clean.shape)
            display(df_e_valid.head())
            print("Edges →", df_e_valid.shape)

        # 4) Now actually add them
        self.add_vertices_from_dataframe(
            df_n_clean,
            id_col=node_id_col,
            layer_col=node_layer_col,
    ...
    def add_vertices_from_dataframe(
        ...
        if property_cols:
            ...
            if collision:
                raise ValueError(
                    f"Cannot use {collision!r} as property name, it's reserved for internal keys",
                )
        # prop-col existence
        if property_cols:
        ...
        if property_cols:
            for prop in property_cols:
                vals = df[prop].values
                typ = (
                    property_types.get(prop)
                    if property_types and prop in property_types
                    else infer_property_type(df[prop])
                )
                if typ in ["int", "float"] and not string_override:
                    ...
                    if n_new:
                        self.core.graph.vp[prop].a[start:] = np.array(
                            [mapped[i] for i in new_rows_idx],
                            dtype=int,
                        )
                    for i, cid in enumerate(custom_ids):
                        if not is_new[i]:
            ...
            for prop in property_cols:
                if prop in self.core.graph.vp:
                    ...
                    if prop in self.core.vertex_categorical_mappings:
                        if (
                            property_types
                            and prop in property_types
                            and property_types[prop] in ("int", "float")
                        ):
                            raise ValueError(
                                f"Vertex property '{prop}' was categorical; cannot ingest as numeric.",
                            )
                    else:
                        ...
                        if (
                            property_types
                            and prop in property_types
                            and property_types[prop] in ("str", "string")
                        ) or string_override:
                            raise ValueError(
                                f"Vertex property '{prop}' was numeric; cannot ingest as categorical.",
                            )
    ...
    def add_edges_from_dataframe(
        ...
        if drop_na:
            ...
            if (
                df[[source_id_col, source_layer_col, target_id_col, target_layer_col]]
                .isna()
                .any()
                .any()
            ):
        ...
        if drop_duplicates:
            ...
            for k, (s, t) in enumerate(zip(src_valid, tgt_valid, strict=False)):
                if consider_props_in_duplicate and property_cols:
                    key = (s, t, *(df_valid.iloc[k][p] for p in property_cols))
                else:
                    key = (s, t)
                if key in seen:
                    continue
                ...
                if prop in self.core.graph.ep:
                    if prop in self.core.edge_categorical_mappings:
                        ...
                        if (
                            ...
                            raise ValueError(
                                f"Edge property '{prop}' was categorical; cannot ingest as numeric.",
                            )
                    else:
                        # previously numeric, forbid switching to categorical
                        if (
                            ...
                            string_override and (not property_types or prop not in property_types)
                        ):
                            raise ValueError(
                                f"Edge property '{prop}' was numeric; cannot ingest as categorical.",
                            )
            for prop in property_cols:

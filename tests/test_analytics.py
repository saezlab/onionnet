...
def test_plot_layer_metagraph_builds_graph_and_calls_draw(monkeypatch):
    ...
    pairs = pd.DataFrame(
        {'edges': [3, 1]},
        index=pd.MultiIndex.from_tuples([('A','B'), ('B','B')], names=['source_layer','target_layer'])
    )
...
def test_plot_layer_metagraph_auto_colors_and_padding(monkeypatch):
    ...
    pairs = pd.DataFrame({'edges':[2,2]}, index=pd.MultiIndex.from_tuples([('alpha_u','alpha_v'), ('beta_x','beta_y')], names=['source_layer','target_layer']))
...
def test_plot_layer_metagraph_no_labels_and_empty_edges(monkeypatch):
    ...
    plot_layer_metagraph(empty_pairs, nodes_by_layer=None, show_labels=False, show_edge_counts=False)
...
def test_layer_stats_edges_without_interlayer_and_uniform_weights():
    ...
    nodes_by_layer, edges_by_pair = layer_stats(df_nodes=df_nodes, df_edges=df_edges, print_tables=False)
...
def test_plot_layer_metagraph_uniform_edge_weights(monkeypatch):
    ...
    pairs = pd.DataFrame({'edges':[5,5]}, index=pd.MultiIndex.from_tuples([('A','B'),('B','A')], names=['source_layer','target_layer']))
    captured = {}
    def fake_draw(g, **kwargs):
        captured['kwargs'] = kwargs
        return None
...
def test_layer_stats_edges_from_core_properties():
    ...
    ep_s = g.new_edge_property('int'); ep_t = g.new_edge_property('int')
    ep_s[e1], ep_t[e1] = 0, 1
    ep_s[e2], ep_t[e2] = 1, 0
    ...
    nodes_by_layer, edges_by_pair = layer_stats(df_nodes=df_nodes, df_edges=None, core=core, print_tables=False)
    # Expect pairs counted from core's edge props
    assert set(edges_by_pair.index.get_level_values(0)) | set(edges_by_pair.index.get_level_values(1)) == {'L1','L2'}

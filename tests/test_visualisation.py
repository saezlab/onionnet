...
def test_load_or_compute_layout_inject_and_load(tmp_path):
    ...
    pos1 = load_or_compute_layout(
        ...
        str(fn),
        override=False,
        inject=lambda G: {v: [0.1 * i, 0.2 * i] for i, v in enumerate(G.vertices())},
    )
    assert fn.exists()
    # now load from file
...
def test_load_or_compute_layout_decoded_keys(tmp_path):
    g = gt.Graph(directed=False)
    g.add_vertex(2)
    ld = g.new_vertex_property("string")
    nd = g.new_vertex_property("string")
    for i, v in enumerate(g.vertices()):
        ld[v] = ["L", "R"][i]
        nd[v] = ["A", "B"][i]
...
def test_missing_rows_in_file_error(tmp_tsv):
    ...
    g = make_simple_graph(3)
    df = pd.DataFrame({"layer_hash": [0, 0], "node_id_hash": [0, 1], "x": [0, 1], "y": [0, 1]})
